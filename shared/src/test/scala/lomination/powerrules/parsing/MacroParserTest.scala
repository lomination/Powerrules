...
import lomination.powerrules.parsing.{Macro, InvalidNumberOfMacroParameters, MacroParser}
...
class UseMacro extends FunSuite {
  ...
  test("UseMacro (1): without params") {
    ...
    val result   = P.applyMacros(marcos, input)
  ...
  test("UseMacro (3): wrong number of params") {
    ...
    val marcos = Seq(Macro("tile", Seq(), "23"))
    val result = Try(P.applyMacros(marcos, input))
    assert(result.isFailure)
    assert(result.failed.get.isInstanceOf[InvalidNumberOfMacroParameters])
  }
